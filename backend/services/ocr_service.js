...
async function runOcrPipeline(imagePath, metadata = {}) {
  ...
  try {
    ...
    if (isTextEmpty && !config.gemini?.enabled) {
      // No fallback available — fail gracefully with user-facing message
      throw Object.assign(
        new Error(
          'No readable text detected on this image. ' +
          'Please try: better lighting, camera closer to the label, avoid glare/blur.'
        ),
        { code: 'NO_TEXT_DETECTED', tesseractConfidence: tesseractResult.confidence }
      );
    }
    ...
    if (needsGeminiFallback) {
      ...
      try {
        const geminiResult = await runGeminiVision(imagePath);
        ...
        return {
          ...
          engine: 'gemini',
          geminiStructuredData: geminiResult.structuredData,
          tesseractRawText: tesseractResult.text,   // kept for debug/logging
          _fontMetrics: tesseractResult._fontMetrics, // use Tesseract bboxes even on Gemini path
        };
    ...
    return {
      ...
      _fontMetrics: tesseractResult._fontMetrics,
    };

  } finally {
    // Always clean up temp preprocessed file
