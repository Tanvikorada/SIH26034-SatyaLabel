...
function RuleFilmstrip() {
  return (
    <div className="relative h-[120px] w-full overflow-hidden bg-charcoal rounded-xl border border-graphite">
      ...
      >
        {["Rule 6(1)(a)", "Rule 7(2)", "Rule 18(1)", "Rule 6(1)(c)", "Rule 9(3)", "Rule 26", "Rule 6(1)(e)", "Rule 4(1)", "Rule 6(1)(f)"].map((r, i) => (
          <div key={i} className="h-[60px] flex items-center justify-center text-mist font-mono text-[14px]">
            {r}
          </div>
        ))}
      </motion.div>
      ...
      {/* Haptic pulse */}
      <motion.div 
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 bg-accent rounded-full z-0 blur-xl mix-blend-screen"
...
export default function LandingPage() {
      ...
      content: (
        <div className="w-full h-[180px] bg-charcoal rounded-xl overflow-hidden relative border border-graphite">
          ...
          <div className="absolute inset-0 scan-line-container">
            <div className="w-full h-1 bg-accent shadow-[0_0_15px_rgba(251,146,60,0.8)] absolute top-0 animate-scan"></div>
          </div>
        </div>
      )
    }
  ...
  const validateSteps = [
    {
      ...
      durationMs: 1500,
      content: (
        <div className="flex flex-col gap-2 w-full text-left font-mono text-[11px] text-pearl">
          <div className="bg-charcoal p-3 rounded-lg border border-graphite">{'{\n  "mrp": "Rs. 150.00",\n  "net_weight": "500g",\n  "date": "10/2023"\n}'}</div>
        </div>
      )
    },
    {
  ...
  const reportSteps = [
    {
      ...
      content: (
        <div className="flex flex-col items-center">
          <div className="w-16 h-20 bg-charcoal border border-graphite rounded shadow-lg flex flex-col p-2 gap-2 relative">
             <div className="w-full h-1 bg-graphite rounded-full"></div>
             <div className="w-3/4 h-1 bg-graphite rounded-full"></div>
             <div className="w-full h-1 bg-graphite rounded-full mt-2"></div>
             <div className="w-1/2 h-1 bg-graphite rounded-full"></div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-obsidian text-pearl font-sans selection:bg-accent/30 relative overflow-hidden">
      
      {/* Background layer */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.03] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white via-obsidian to-obsidian z-0" style={{ backgroundSize: '24px 24px', backgroundImage: 'radial-gradient(circle, #ffffff 1px, transparent 1px)' }} />
      ...
      <nav className="w-full flex items-center justify-between px-6 py-4 md:px-12 relative z-10">
        ...
        <div className="hidden md:flex items-center gap-8 text-[14px] font-medium text-fog">
      ...
      >
        ...
        <motion.p variants={childVariants} className="text-[18px] md:text-[20px] text-mist mb-12 max-w-[640px] leading-relaxed">
      ...
      <motion.section 
        id="how-it-works" 
        className="py-24 px-6 md:px-12 bg-obsidian border-y border-graphite relative z-10"
        initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={sectionVariants}
      >
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 bg-charcoal rounded-[24px] border border-graphite overflow-hidden shadow-2xl">
          {/* Left: Manual */}
          <motion.div 
            className="p-10 border-b md:border-b-0 md:border-r border-graphite flex flex-col justify-center items-center text-center opacity-70"
            variants={{ hidden: { opacity: 0, x: -20 }, visible: { opacity: 0.7, x: 0, transition: { type: "spring", stiffness: 300, damping: 30 } } }}
          >
            <div className="w-16 h-16 rounded-full bg-obsidian border border-graphite flex items-center justify-center mb-6">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-fog"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            </div>
            <h3 className="text-[20px] font-medium text-white mb-2">Manual Inspection</h3>
            <p className="text-[15px] text-fog">15-30 minutes per product. Error-prone. Hard to enforce uniformly at scale.</p>
          </motion.div>

          {/* Right: Automated */}
          <motion.div 
            className="p-10 flex flex-col justify-center items-center text-center bg-obsidian/50 relative overflow-hidden"
          ...
          >
            ...
            <div className="w-16 h-16 rounded-full bg-accent/10 border border-accent/30 flex items-center justify-center mb-6 relative z-10 shadow-[0_0_15px_rgba(251,146,60,0.2)]">
            ...
            <h3 className="text-[20px] font-medium text-white mb-2 relative z-10">SatyaLabel</h3>
            <p className="text-[15px] text-mist relative z-10">2-4 seconds per product. Perfectly consistent. Scales to millions of SKUs.</p>
          </motion.div>
        </div>
      </motion.section>
      ...
      >
        ...
        <motion.div variants={childVariants} className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-20">
          {[
            ...
            { badge: "NOT VERIFIED", class: "bg-charcoal border border-graphite text-[#fb923c] px-[10px] py-[4px] rounded-full text-[12px] font-medium inline-block", desc: "Text missing or unreadable." },
          ].map((status, i) => (
            <motion.div 
              key={i} 
            ...
            >
              ...
              <p className="text-[13px] text-fog leading-relaxed">{status.desc}</p>
            </motion.div>
          ))}
        </motion.div>

        {/* Rule Citation Example */}
        <motion.div variants={childVariants} className="mello-card-flat p-8 relative overflow-hidden bg-obsidian">
          <div className="absolute inset-0 bg-gradient-to-r from-charcoal to-transparent opacity-50" />
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
            <div className="flex-1 bg-charcoal p-4 rounded-xl border border-graphite font-mono text-[13px] text-pearl shadow-inner w-full">
              <div className="text-fog mb-2">// Raw Extracted Text</div>
              <div className="text-mist bg-obsidian p-3 rounded border border-graphite/50 mb-2 overflow-x-auto">
                &quot;MRP Rs. 150.00&quot;
              </div>
              <div className="text-fog mb-2">// Parsed Data</div>
              <div className="text-mist bg-obsidian p-3 rounded border border-graphite/50 overflow-x-auto">
            ...
            <div className="flex-1 w-full">
              <h3 className="text-[14px] font-medium uppercase tracking-widest text-fog mb-4">Deterministic Logic</h3>
              <div className="mello-card-flat bg-charcoal/50 p-5 border-l-2 border-l-accent">
                ...
                <p className="text-[14px] text-mist leading-relaxed mt-3">
      ...
      <motion.section 
        id="tech" 
        className="py-24 px-6 md:px-12 bg-obsidian border-t border-graphite relative z-10"
        initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={sectionVariants}
      >
      ...
      {/* Footer */}
      <footer className="w-full py-8 px-6 md:px-12 border-t border-graphite flex flex-col md:flex-row justify-between items-center gap-4 text-[13px] text-fog relative z-10 bg-obsidian">
