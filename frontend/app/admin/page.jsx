        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {highRiskBrands.map((brand, idx) => (
                    <div key={idx} className="glass rounded-[16px] p-4 border border-red-500/20 relative overflow-hidden group">
                      ...
                      <div className="flex justify-between items-start mb-2 relative">
                        <div>
                          <h3 className="font-bold text-white text-[15px]">{brand.product_name}</h3>
                          <p className="text-[12px] text-slate-400">{brand.brand_name || 'Unknown Manufacturer'}</p>
                        </div>
                        <span className="bg-red-500/10 text-red-400 text-[10px] px-2 py-0.5 rounded-sm border border-red-500/20 font-bold tracking-wider">
                          CRITICAL
                        </span>
                      </div>
                      <div className="mt-3">
                        <div className="flex justify-between text-[11px] mb-1">
                          <span className="text-slate-400">Total Scans: {brand.total_scans}</span>
                          <span className="text-red-400 font-medium">Violations: {brand.violations}</span>
                        </div>
                  ...
                  {highRiskBrands.length === 0 && (
                    <div className="col-span-2 glass rounded-[16px] p-8 text-center text-slate-400 border border-[var(--color-border)]">
            ...
            <div className="space-y-6">
              ...
              <div className="glass rounded-[20px] p-2 border border-[var(--color-border)] relative">
                <div className="w-full h-[450px] rounded-[14px] overflow-hidden relative bg-slate-900">
                  {mapData.length > 0 ? (
                    <MapWidget 
                      markers={mapData} 
                      height="100%" 
              ...
              <button 
                ...
                className={`w-full py-3.5 rounded-[14px] font-bold text-[14px] flex items-center justify-center gap-2 transition-all ${
                  ...
                  : 'bg-gradient-to-r from-red-600 to-rose-500 text-white shadow-[0_0_20px_rgba(225,29,72,0.4)] hover:shadow-[0_0_30px_rgba(225,29,72,0.6)] border border-red-400/50'
                }`}
              >
                {isDeploying ? (
                  <>
