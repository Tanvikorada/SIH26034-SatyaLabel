...
export default function PublicReportPage() {
    ...
    return (
      <div className="min-h-screen bg-background text-text-primary flex flex-col items-center justify-center p-6 text-center relative overflow-hidden">
        ...
        <div className="z-10 animate-fade-in glass p-8 rounded-3xl max-w-sm flex flex-col items-center">
          ...
          <button onClick={() => {setSubmitted(false); setFile(null); setPreview(null);}} className="btn btn-secondary w-full py-3">Submit Another</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background text-text-primary flex flex-col items-center justify-center p-4 relative overflow-hidden">
      <div className="orb-saffron"></div>
      <div className="orb-blue"></div>
      <div className="w-full max-w-md glass rounded-[24px] p-6 shadow-2xl z-10 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="relative w-full aspect-[4/3] rounded-xl overflow-hidden bg-[var(--color-surface)] border-2 border-dashed border-[var(--color-border)] hover:border-accent transition-colors flex flex-col items-center justify-center group cursor-pointer">
            {preview ? (
              ...
              <div className="flex flex-col items-center gap-2 p-6 text-center pointer-events-none">
                ...
                <span className="text-sm font-medium text-text-primary">Tap to Take Photo</span>
          ...
          <button 
            ...
            className={`btn w-full py-4 text-sm tracking-wide ${!file ? 'opacity-50 cursor-not-allowed bg-[var(--color-surface)] text-text-muted' : 'btn-primary shadow-[0_0_20px_rgba(245,158,11,0.2)]'}`}
          >
            {loading ? (
