      ...
      <div className="w-full max-w-md bg-slate-800 rounded-[24px] p-6 shadow-2xl border border-slate-700">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <button 
            ...
            className={`w-full py-4 rounded-xl text-sm font-bold tracking-wide flex justify-center items-center gap-2 transition-all ${!file ? 'bg-slate-700 text-slate-500' : 'bg-gradient-to-r from-amber-500 to-orange-600 text-white shadow-[0_0_20px_rgba(245,158,11,0.4)] hover:shadow-[0_0_30px_rgba(245,158,11,0.6)]'}`}
          >
            {loading ? (
        ...
        <div className="mt-8 pt-4 border-t border-slate-700 flex justify-between items-center text-[10px] text-slate-500 uppercase tracking-widest">
          ...
          <span className="flex items-center gap-1">
            <span className={`w-1.5 h-1.5 rounded-full ${location ? 'bg-emerald-500' : 'bg-slate-600'}`}></span> GPS {location ? 'Locked' : 'Off'}
          </span>
        </div>
      </div>
    </div>
