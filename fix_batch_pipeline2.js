...
const regex = /async function runBatchPipeline\(batch, imagePath, metadata = \{\}\) \{[\s\S]*?\n  \}\n  \/\/ OLD:/g;
...
if (regex.test(js)) {
  ...
  console.log("Completely rewrote runBatchPipeline!");
} else {
  console.log("Could not find the function via regex.");
}
